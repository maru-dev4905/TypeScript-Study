...
}; // Error: 'year' 속성이 '{ place: string; }' 형식에 없지만 'Setting' 형식에서 필수이다.
...
// 이렇게 하면 파생 인터페이스(derived interface)를 준수하는 모든 객체가 기본 인터페이스의 모든 멤버도 가져야 한다는 것을 타입스크립트에 알려준다.
...
let missingPages: Novella = {
  title: '딸기맛 쿠키의 먹방 비법',
}; // Error: 'pages' 속성이 '{title: string;}' 형식에 없지만 'Novella' 형식에서 필수이다.

let extraProperty: Novella = {
  title: '마법사맛 쿠키의 마법약 제조법',
  pages: 192,
  style: 'moon',
}; // Error: 객체 리터럴은 알려진 속성만 지정할 수 있으며 'Novella' 형식에 'style'이 없다.

// 3.1 재정의된 속성
// 파생 인터페이스는 다른 타입으로 속성을 다시 선언해 기본 인터페이스의 속성을 재정의 하거나 대체할 수 있다.
// 타입스크립트의 타입 검사기는 재정의된 속성이 기본 속성에 할당되어 있도록 강요한다.
// 이렇게 하면 파생 인터페이스의 타입의 인스턴스를 기본 인터페이스 타입에 할당할 수 있다.

interface WithNullableName {
  name: string | null;
}

interface WithNonNullabeldName extends WithNullableName {
  name: string;
}
...
// 3.2 다중 인터페이스 확장
// 타입스크립트의 인터페이스는 여러 개의 다른 인터페이스를 확장해서 선언할 수 있따.
// 파생 인터페이스 이름에 있는 extends 키워드 뒤에 쉼표로 인터페이스의 이름을 구분해 사용하면 된다.
