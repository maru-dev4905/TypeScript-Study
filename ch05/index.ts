...
// 4.3 튜플 추론
// 타입스크립트는 생성된 배열을 튜플이 아닌 가변 길이의 배열로 취급한다. 배열이 변수의 초깃값 또는 함수에 대한 반환값으로 사용되는 경우,
// 고정된 크기의 튜플이 아니라 유연한 크기의 배열로 가정한다.
...
// 4.3.2 const 어서션
// 명시적 타입 애너테이션에 튜플 타입을 입력하는 작업은 타입 애너테이션을 입력하는 때와 동일한 이유로 번거로울 수 있다.
...
// 타입: (string | number)[]
const unionArray = [115, '용감한 쿠키'];

// 타입: readonly [115, '용감한 쿠키];
const readonlyTuple = [115, '용감한 쿠키'] as const;
// readonlyTuple[1] = '딸기맛 쿠키';
// readonlyTuple.push('딸기맛 쿠키');

// const 어서션은 유연한 크기의 배열을 고정된 크기의 튜플로 전환하는 것을 넘어서, 해당 튜플이 읽기 전용이고 값 수정이 예상되는 곳에서 사용할 수 없음을 나타낸다.

const pairMutable: [number, string] = [110, '진저 브레드'];
pairMutable[0] = 1000; // OK

const readonlyPair = [220, '딸기맛 쿠키'] as const;
const pairAlsoMutable: [number, string] = readonlyPair; // Error: 'readonly [220, '딸기맛 쿠키']' 형식은 'readonly'이며 변경 가능한 형식 '[number, string]'에 할당할 수 없다.

const pairConst = [142, '마법사맛 쿠키'] as const;
pairConst[0] = 5000; // Error: 읽기 전용 속성이므로 '0'에 할당할 수 없다.